//ui.js将依赖下面的文件，请不要修改
document.write('<script src="/Js/UI/dataBind.js" type="text/javascript"></script>');
document.write('<script src="/Js/dialog.js" type="text/javascript"></script>');
document.write('<link rel="stylesheet" href="/Js/UI/ui.css">')
//检测ie低版浏览器
...
$(document).ready(function () {
    ...
    $(document).on("click", ".ui-deleteSingle", defaultParams, function (event) {
        ...
        //confirm逻辑
        if (confirmMsg != "") {
            var postConfirm = $this.data("confirm");
            if (postConfirm == undefined) {
                $this.data("confirm", 0);
                postConfirm = 0;
            }
            if (postConfirm == 0) {
                Confirm(confirmMsg, { icon: 3 }, function () { $this.data("confirm", 1); $this.click(); });
                return;
            }
            $this.data("confirm", 0);
        }
...
$(document).ready(function () {
    ...
    $pagePost.off("click").on("click", function () {
        ...
        if (confirmMsg != "") {
            ...
            $this.removeData("confirm");//删除confirm data
        }

        var $convertMultipleSelect = $form.find(".ui-convertSelect");
...
$(document).ready(function () {
    ...
    $setMultiple.click(function () {
        ...
        var defaultParams = { title: "标题", setName: "", url: "", ids: "", itemPrefix: "#item_", accordingObj: ".checked-item", additionals: "", beforeExecute: "", addParams: "", width: "100%", height: "100%", target: 'self', shadeClose: true, limitLevel: "", limitFinal: "", includeChildNode: "", nullMsg: "请选择要操作的项!" };
        var thisParam = $this.attr("data-custom-params");
        defaultParams = ExtendObject(defaultParams, thisParam);
        var ids = defaultParams.ids;
        if (typeof (ids) == "undefined") { ids = ""; }
...
$(document).ready(function () {
    ...
    $setField.click(function () {
        var $this = $(this);
        var defaultParams = { url: "", setField: "", setValue: "", ids: "", setName: "", itemPrefix: "#item_", accordingObj: ".checked-item", beforeExecute: "", callBack: "", successMsg: "操作成功!", limitLevel: "", limitFinal: "", includeChildNode: "" };
        var thisParam = $this.attr("data-custom-params");
        defaultParams = ExtendObject(defaultParams, thisParam);
        var ids = defaultParams.ids;
        if (typeof (ids) == "undefined") { ids = ""; }
...
$(document).ready(function () {
    var defaultParams = { type: "get", url: "", accordingObj: '', additionals: "", async: true, beforeExecute: "", callBack: "", confirmMsg: "是否确定提交？", successMsg: "提交成功！" };
    $(document).on("click", ".ui-ajaxRequest", defaultParams, function (event) {
        ...
        if (accordingObj != "") {
            ...
            paramsData = $item.Serializes(additionals);
        }
        Loading();
...
$(document).ready(function () {
    ...
    $uploadPanel.each(function () {
        _InitUploadPanel($(this));
    });
});

//初始化上传面板
function _InitUploadPanel(jqObj) {
    var $this = jqObj;
    ...
    var defaultParams = { fileType: 'image', fileNumLimit: 1, uploadPanelUrl: "/E/UploadPanel" }
    ...
    if (!initialization) {
        ...
        if (fileType == "images" || fileType == "image") {
            boxHtml = "<div class=\"upload-attachments-list\"><dl><dd class=\"item hidden\" data-path=\"{path}\" data-thumbnail=\"{thumbnail}\" data-name=\"{name}\"><a href=\"{path}\" target=\"_blank\"  title=\"{name}\"><img src=\"{thumbnail}\"></a><span class=\"lb-delete-box\"><i class=\"btn-delete fa fa-trash-o\"  title=\"删除\"></i></span></dd>";
            boxHtml += "<dd class=\"btn-upload\"><a href=\"javascript:void(0)\" title=\"添加\"><i class=\"fa fa-plus\"></i><a></dd></dl></div>"
        }
        else if (fileType == "file")
        {
            boxHtml ="<span class=\"input-group-btn btn-upload\"><button class=\"btn btn-default btn-sm\" type=\"button\">上传</button></span>"
        }
        else if (fileType == "files"){
            boxHtml = "<div class=\"upload-attachments-list upload-attachments-file-list\"><dl><dd class=\"item hidden\" data-path=\"{path}\" data-thumbnail=\"{thumbnail}\" data-name=\"{name}\"><a href=\"{path}\" target=\"_blank\"  title=\"{name}\">{name}</a><span class=\"lb-delete-box\"><i class=\"btn-delete fa fa-trash-o\"  title=\"删除\"></i></span></dd>";
            boxHtml += "<dd class=\"btn-upload\"><a href=\"javascript:void(0)\" title=\"添加\"><i class=\"fa fa-plus\"></i><a></dd></dl></div>"
        }
        $this.after(boxHtml);
        if (fileType == "file")
    ...
    if (fileType == "image" || fileType == "images" || fileType == "files") {
        ...
        $btnButton = $this.siblings(".btn-upload");
    }

    var uploadUrl = defaultParams.uploadPanelUrl + "?objName=" + UrlEncode("#" + objId);
    if (!initialization) {
        $btnButton.click(function () {
            var params = { title: "上传界面", url: uploadUrl, width: "700px", height: "400px", target: "self" }
            IDialog(params);
        });
    }
        ...
        $attachments.on("click", ".btn-delete", options, function (event) {
            ...
            if (fileType == "image" || fileType == "file") {
                ...
                $items.each(function (index) {
                    var $this = $(this);
                    var json = { "path": $this.attr("data-path"), "thumbnail": $this.attr("data-thumbnail"), "name": $this.attr("data-name") };
...
; (function ($, window, undefined) {
    $.fn.CreatePagePanel = function (pageInfo, customPagesize) {
        ...
        html += '<span class="right"><span class="lb_recordcount" data-recordCount="' + recordCount + '">共' + recordCount + '条</span> ';
        //html += '<span class="lb_currentpage" currentPage="' + currentPage + '" pageCount="' + pageCount + '">当前页次: ' + currentPage + '/' + pageCount + '</span> ';
        html += '<button type="button" class="btn btn-sm btn-default ui-prevpage" ' + (currentPage == "1" ? "disabled" : "") + ' onclick="_PageSearch(\'-\')">上一页</button> ';
        html += '<button type="button" class="btn btn-sm btn-default ui-nextpage" ' + (currentPage == pageCount ? "disabled" : "") + ' onclick="_PageSearch(\'+\')">下一页</button> ';
        html += '<select id="page" name="page" class="form-control ui-queryString" style="border:1px solid #cccccc;width:auto;display:inline" onchange="_PageSearch()">';
        ...
        if (customPagesize) {
            ...
            html += ' <select id="pagesize" name="pagesize" class="form-control ui-queryString" style="border:1px solid #cccccc;width:auto;display:inline" onchange="_PageSearch()">';
...
function _InitListXuhao() {
    ...
    $inputpaixu.each(function (i) {
        var str = "<a href=\"javascript:void(0)\" onclick=\"ListPaiXu(this,'up')\" title=\"向上移动\" style=\"padding:0px 2px;font-size:14px;\">↑</a>";
...
function ListPaiXu(obj, act) {
    ...
    {
        var ajaxOptions = { type: "post", dataType: "text", async: true, url: "/E/UpdateListXuhao/", data: { table: table, field: field, ids: ids, values: values } };
        Ajax(ajaxOptions);
    }
...
$(document).ready(function () {
    ...
    $passwordEye.each(function () {
        ...
        var str = "<i class=\"fa fa-eye-slash ui-password-eye-icon\"></i>"; 
